...
calving_season_daily_trim <- calving_season_daily_trim %>%
  group_by(calving_season) %>%
  arrange(date) %>%
  mutate(day_of_season = row_number()) %>% #now 1 is january 1st
  ungroup()
...
Most years are significantly cooler than abnormal year (coloured red), but specifically over the mid winter months and early spring (jan to end of april) this year is warmer. It's cooler earlier in the winter/late fall though.
```{r visualizations}
...
### Narrower date window (January to end of April)
If we narrow the window to only january-april? These are the months that appear warmer from the first gamm.
All are significantly cooler than abnormal year in this time period.

Day of season has been changed to a count that starts January 1st.
```{r targeted gamm}
# Fit a Generalized Additive Mixed Model (GAMM) with AR(1) autocorrelation within calving season
gam_model_trim <- gamm(mean_temp ~ s(day_of_season, by=calving_season) + precip_amt + calving_season, 
...
gamm_plot_trim <- ggplot(calving_season_daily_trim, aes(x = day_of_season, y = mean_temp, color = calving_season)) +
  ...
  scale_x_continuous(breaks = c(0, 31, 59, 90, 120), labels = c( 'Jan', 'Feb', 'Mar', 'Apr', 'May')) +
  labs(x = "Month", y = "Mean Temperature (°C)",
       title = "Modeled Temperature Trends Across Calving Seasons") +
  theme_minimal()
...
all estimates are negative when compared to the abnormal year, indicating that year was warmer. However, the two more recent years (2021/2022, 2022/2023) were not significantly cooler (until using the trimmed date window). However they are cooler from January to the beginning of May like the other years. This narrows down the time window for the cue for early casting and growth.
...
Maybe look into changes in habitat selection and variability in temperature, or just stick with average daily temperature?
